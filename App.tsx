...
import { Copy, RefreshCw, Lock, ShieldCheck, Cpu, Hash, Network, Check } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const generate = useCallback(async () => {
    ...
    // Play sound immediately for feel
    audioService.playGenerateSound();

    try {
      // Simulate a tiny delay for the "processing" feel even for local generation
  ...
  return (
    <div className="min-h-screen text-slate-100 flex items-center justify-center p-4 relative font-sans">
      ...
      <main className="w-full max-w-2xl bg-slate-900/70 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden relative z-10 animate-in fade-in zoom-in duration-500">
        
        {/* Header */}
        <div className="p-8 border-b border-white/5 flex flex-col sm:flex-row items-center justify-between bg-gradient-to-r from-blue-500/10 to-purple-500/10 gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-blue-200/70 text-xs tracking-wider uppercase">安全熵值生成器</p>
            </div>
          </div>
          <div className="flex gap-2 bg-slate-800/50 p-1 rounded-xl">
            {[
              ...
              { id: GeneratorMode.MAC, icon: Network, label: 'MAC地址' },
            ].map((tab) => (
              <button
                key={tab.id}
                onClick={() => setMode(tab.id as GeneratorMode)}
                className={`flex flex-col items-center justify-center px-3 py-2 rounded-lg transition-all duration-300 border min-w-[60px] ${
                  mode === tab.id
        ...
        <div className="p-8 pb-4 relative group">
           ...
           >
             ...
             <div className="bg-black/40 border border-white/10 rounded-2xl p-6 min-h-[140px] flex items-center justify-center text-center transition-all duration-300 group-hover/display:border-blue-500/50 group-hover/display:shadow-[0_0_30px_rgba(59,130,246,0.15)] group-active/display:scale-[0.99]">
                {loading ? (
                   <div className="flex flex-col items-center gap-3 animate-pulse">
                     ...
                     <span className="text-sm text-blue-300 font-mono tracking-widest">计算熵值中...</span>
                   </div>
                ) : (
                  <div className="relative w-full">
                    <p className="font-mono text-3xl md:text-4xl break-all text-white font-semibold tracking-tight drop-shadow-[0_0_15px_rgba(255,255,255,0.3)] select-none">
                      {generated.text}
                    </p>
                    {/* Floating 'Copied' feedback centered over text */}
                    {copied && (
             ...
             {!loading && !copied && (
               <div className="absolute bottom-2 left-0 right-0 text-center opacity-0 group-hover/display:opacity-100 transition-opacity duration-300">
                  <span className="text-xs text-blue-300/60 bg-black/40 px-3 py-1 rounded-full border border-blue-500/20">点击复制密码</span>
               </div>
             )}
           </div>
